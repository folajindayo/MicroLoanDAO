        ...
        <div className="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg border border-gray-200 dark:border-gray-700 transition-shadow hover:shadow-md" role="article" aria-labelledby={`loan-title-${loan.id}`}>
            <div className="px-4 py-5 sm:p-6">
                <dl>
                    ...
                    <dd className="mt-1 text-3xl font-semibold text-gray-900 dark:text-white tracking-tight">{formatEther(BigInt(loan.amount))} ETH</dd>
                    
                    <dt className="mt-4 text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Purpose</dt>
                    <dd id={`loan-title-${loan.id}`} className="mt-1 text-sm text-gray-900 dark:text-gray-200">{loan.purpose}</dd>
                ...
                {loan.status === 'REQUESTED' && userAddress && loan.borrowerAddress !== userAddress && (
                    <button
                        ...
                        aria-busy={isWritePending && fundingLoanId === loan.id}
                    >
