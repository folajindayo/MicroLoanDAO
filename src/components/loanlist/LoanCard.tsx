...
export default function LoanCard({ loan, userAddress, onFund, isWritePending, fundingLoanId }: LoanCardProps) {
  return (
    <div className="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg border border-gray-200 dark:border-gray-700 p-4 transition-all hover:shadow-md">
      <div className="px-4 py-5 sm:p-6">
        <dl>
          <dt className="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Borrower</dt>
          <dd className="mt-1 text-sm text-gray-900 dark:text-gray-200 truncate font-mono">{loan.borrowerAddress}</dd>
        ...
        {loan.status === 'REQUESTED' && userAddress && loan.borrowerAddress !== userAddress && (
            <button
              ...
              className="mt-6 w-full inline-flex items-center justify-center px-4 py-2 border border-transparent font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50 transition-colors shadow-sm"
            >
